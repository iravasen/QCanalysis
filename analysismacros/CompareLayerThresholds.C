...
void DoAnalysis(string filepath, const int nChips, bool isIB, string skipruns, long int refrun){
  ...
  for(int ilay=0; ilay<nLayers; ilay++)
    hCorr[ilay] = new TH2D(Form("hCorr_L%s",laynums[ilay*nRuns].c_str()), Form("Layer-%s - THR corr. %s - Ref. run: %ld; Chip Threshold (run%ld); Chip Threshold (runs)",laynums[ilay*nRuns].c_str(),filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str(),refrun,refrun), 150, 7.5, 13.5, 150, 7.5, 13.5);
  ...
  for(int ilay=0; ilay<nLayers; ilay++){
    ...
    canvas->SaveAs(Form("../Plots/Layer%s_thresholdcorr_refrun%ld_%s.pdf", laynums[ilay*nRuns].c_str(), refrun, filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
    canvas->SaveAs(Form("../Plots/Layer%s_thresholdcorr_refrun%ld_%s.root", laynums[ilay*nRuns].c_str(), refrun, filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
    delete canvas;
  }
