...
void DoAnalysis(string filepath, const int nChips, string skipruns, bool isIB){
  ...
  while((key = ((TKey*)next()))){
    ...
    if(objtitle.find("DeadPixel")!=string::npos) break;//take only maps of avg thresholds
  ...
  for(int ilay=0; ilay<nLayers; ilay++){
    ...
    leg->Draw("same");
    canvas->SaveAs(Form("../Plots/Layer%s_thresholds_%s.pdf", laynums[ilay*nRuns].c_str(), filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
    canvas->SaveAs(Form("../Plots/Layer%s_thresholds_%s.root", laynums[ilay*nRuns].c_str(), filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
    delete canvas;
    delete leg;
  }
  ...
  for(int ilay=0; ilay<nLayers; ilay++)//remove images if they exist already
    gSystem->Unlink(Form("../Plots/Layer%s_thresholdmap_%s.gif", laynums[ilay*nRuns].c_str(), filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
  for(int ihist=(int)hmaps.size()-1; ihist>=0; ihist--){// start from the last in order to have the runs from the oldest to the newest
    TCanvas *canvas = new TCanvas();
    canvas->cd();
    canvas->SetTickx();
    ...
    hmaps[ihist]->SetMinimum(8.5);
    hmaps[ihist]->SetMaximum(12.5);
    hmaps[ihist]->GetZaxis()->SetTitle("Avg. Threshold (DAC)");
    hmaps[ihist]->SetTitle(Form("Layer-%s, Run %06d (%d/%d)", laynums[ilayer*nRuns].c_str(), stoi(runnumbers[ihist]), irun,nRuns));
    canvas->Print(Form("../Plots/Layer%s_thresholdmap_%s.gif+40", laynums[ilayer*nRuns].c_str(), filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
    irun++;
    if(!ihist && nLayers==1){
      canvas->Print(Form("../Plots/Layer%s_thresholdmap_%s.gif++40++", laynums[ilayer*nRuns].c_str(), filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
      break;
