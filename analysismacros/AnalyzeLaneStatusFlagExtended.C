...
//	Int_t col[] = {TColor::GetColor("#ff3300"), TColor::GetColor("#ec6e0a"), TColor::GetColor("#daaa14"), TColor::GetColor("#c7e51e"), TColor::GetColor("#85dd69"), TColor::GetColor("#42d6b4"), TColor::GetColor("#00ceff"), TColor::GetColor("#009adf"), TColor::GetColor("#0067c0"), TColor::GetColor("#0033a1")};
	...
	for(int iRun=0; iRun<NRun; iRun++){
	  htimeratio[iRun] = new TH1D(Form("timeratio%d",iRun),Form("timeratio%d",iRun),432,0,432);
		...
		for(int iFid=0; iFid<432; iFid++){
			...
			if (Norbit>0) hbfNOK /= (Norbit * laneinfeeid);
			else hbfNOK = -1;
	...
	for(int iStatus=0; iStatus<NStatus; iStatus++){
		for(int iLayer=0; iLayer<NLayer; iLayer++){
			for(int iStave=0; iStave<nstave[iLayer]; iStave++){
				if(iLayer==0) { grt_L0[iStatus][iStave] = new TGraph(); SetStyle(grt_L0[iStatus][iStave], col[iStave%10], 24+iStave/10); }
				if(iLayer==1) { grt_L1[iStatus][iStave] = new TGraph(); SetStyle(grt_L1[iStatus][iStave], col[iStave%10], 24+iStave/10); }
				if(iLayer==2) { grt_L2[iStatus][iStave] = new TGraph(); SetStyle(grt_L2[iStatus][iStave], col[iStave%10], 24+iStave/10); }
				if(iLayer==3) { grt_L3[iStatus][iStave] = new TGraph(); SetStyle(grt_L3[iStatus][iStave], col[iStave%10], 24+iStave/10); }
				if(iLayer==4) { grt_L4[iStatus][iStave] = new TGraph(); SetStyle(grt_L4[iStatus][iStave], col[iStave%10], 24+iStave/10); }
				if(iLayer==5) { grt_L5[iStatus][iStave] = new TGraph(); SetStyle(grt_L5[iStatus][iStave], col[iStave%10], 24+iStave/10); }
				if(iLayer==6) { grt_L6[iStatus][iStave] = new TGraph(); SetStyle(grt_L6[iStatus][iStave], col[iStave%10], 24+iStave/10); }
	...
	for(int iLayer=0; iLayer<NLayer; iLayer++){
		for(int iStave=0; iStave<nstave[iLayer]; iStave++){
			if(iLayer==0) { time_L0[iStave] = new TGraph(); SetStyle(time_L0[iStave], col[iStave%10], 24+iStave/10); }
			if(iLayer==1) { time_L1[iStave] = new TGraph(); SetStyle(time_L1[iStave], col[iStave%10], 24+iStave/10); }
			if(iLayer==2) { time_L2[iStave] = new TGraph(); SetStyle(time_L2[iStave], col[iStave%10], 24+iStave/10); }
			if(iLayer==3) { time_L3[iStave] = new TGraph(); SetStyle(time_L3[iStave], col[iStave%10], 24+iStave/10); }
			if(iLayer==4) { time_L4[iStave] = new TGraph(); SetStyle(time_L4[iStave], col[iStave%10], 24+iStave/10); }
			if(iLayer==5) { time_L5[iStave] = new TGraph(); SetStyle(time_L5[iStave], col[iStave%10], 24+iStave/10); }
			if(iLayer==6) { time_L6[iStave] = new TGraph(); SetStyle(time_L6[iStave], col[iStave%10], 24+iStave/10); }
			...
			for(int iRun=0; iRun<NRun; iRun++){
			  ...
			  if (iLayer == 0) {
			    for (int ifee=binedge_L0[iStave]; ifee<binedge_L0[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L0[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
			  if (iLayer == 1) {
			    for (int ifee=binedge_L1[iStave]; ifee<binedge_L1[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L1[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
			  if (iLayer == 2) {
			    for (int ifee=binedge_L2[iStave]; ifee<binedge_L2[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L2[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
			  if (iLayer == 3) {
			    for (int ifee=binedge_L3[iStave]; ifee<binedge_L3[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L3[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
			  if (iLayer == 4) {
			    for (int ifee=binedge_L4[iStave]; ifee<binedge_L4[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L4[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
			  if (iLayer == 5) {
			    for (int ifee=binedge_L5[iStave]; ifee<binedge_L5[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L5[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
			  if (iLayer == 6) {
			    for (int ifee=binedge_L6[iStave]; ifee<binedge_L6[iStave+1]; ifee++) 
			      if (htimeratio[iRun]->GetBinContent(ifee+1) > -1) {den++; num += htimeratio[iRun]->GetBinContent(ifee+1);}
			    time_L6[iStave]->SetPoint(iRun, iRun, num/den+1.e-8);
			  }
	...
	//	legLayer[iLayer]->SetHeader(Form("Layer%d Stave",iLayer));
		...
		/*
		for(int iStave=0; iStave<tempstave; iStave++){
			if(iLayer==0)legLayer[iLayer]->AddEntry(time_L0[iStave], Form(" %d",iStave),"p");
			if(iLayer==1)legLayer[iLayer]->AddEntry(time_L1[iStave], Form(" %d",iStave),"p");
			if(iLayer==2)legLayer[iLayer]->AddEntry(time_L2[iStave], Form(" %d",iStave),"p");
			if(iLayer==3)legLayer[iLayer]->AddEntry(time_L3[iStave], Form(" %d",iStave),"p");
			if(iLayer==4)legLayer[iLayer]->AddEntry(time_L4[iStave], Form(" %d",iStave),"p");
			if(iLayer==5)legLayer[iLayer]->AddEntry(time_L5[iStave], Form(" %d",iStave),"p");
			if(iLayer==6)legLayer[iLayer]->AddEntry(time_L6[iStave], Form(" %d",iStave),"p");
		}
		*/
	}
	...
	if(ccdb_upload){
			...
			cout<<"Just added to pdf file: Status:"<<StatusKind[iStatus]<<"  Layer:"<<iLayer<<endl;
        ...
        TH1F *hblank2[NLayer];
	for(int iLayer=0; iLayer<NLayer; iLayer++){
	  hblank2[iLayer] = new TH1F(Form("hblank2_%d",iLayer), Form("Layer %d - Percentage of orbits into ERROR or FAULT; Run; Time ratio", iLayer), NRun, -0.5, (double)NRun-0.5);
		for(int ir=0; ir<(int)runnumbers1.size(); ir++)
			hblank2[iLayer]->GetXaxis()->SetBinLabel(ir+1, Form("run%06d", stoi(runnumbers1[runnumbers1.size()-1-ir])));//runnumbers1 is a descending order
	
		hblank2[iLayer]->GetYaxis()->SetRangeUser(1.e-8, 1);
		hblank2[iLayer]->GetXaxis()->SetTitleOffset(2.8);
		ctrend3[iLayer]= new TCanvas(); 
		ctrend3[iLayer]->cd();
		ctrend3[iLayer]->SetTickx();
		ctrend3[iLayer]->SetTicky();
		//ctrend3[iLayer]->SetLogy();
		ctrend3[iLayer]->SetMargin(0.0988,0.1,0.194,0.0993);
		hblank2[iLayer]->Draw();
		gPad->SetLogy();//for total number of errors
		gPad->SetGridx();
	
		if(iLayer==0){for(int iStave=0; iStave<nstave[0]; iStave++){time_L0[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L0[iStave]->Draw("P");}}
		if(iLayer==1){for(int iStave=0; iStave<nstave[1]; iStave++){time_L1[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L1[iStave]->Draw("P");}}
		if(iLayer==2){for(int iStave=0; iStave<nstave[2]; iStave++){time_L2[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L2[iStave]->Draw("P");}}
		if(iLayer==3){for(int iStave=0; iStave<nstave[3]; iStave++){time_L3[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L3[iStave]->Draw("P");}}
		if(iLayer==4){for(int iStave=0; iStave<nstave[4]; iStave++){time_L4[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L4[iStave]->Draw("P");}}
		if(iLayer==5){for(int iStave=0; iStave<nstave[5]; iStave++){time_L5[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L5[iStave]->Draw("P");}}
		if(iLayer==6){for(int iStave=0; iStave<nstave[6]; iStave++){time_L6[iStave]->SetName(Form("L%d_St%d",iLayer,iStave));time_L6[iStave]->Draw("P");}}
		legLayer[iLayer]->Draw();
		//if(ccdb_upload){ // TODO: CHECK THIS
		//string Runperiod = Form("%s",filepath.substr(filepath.find("from"),27).c_str());
		//ctrend3[iLayer]->SetName(Form("Layer%d",iLayer));
		//auto mo2= std::make_shared<o2::quality_control::core::MonitorObject>(ctrend2[iStatus][iLayer], TaskName+Form("/time"), TaskClass, DetectorName,1,Runperiod);
		//mo->setIsOwner(false);
		//ccdb->storeMO(mo);
		//}
		ctrend3[iLayer]->SaveAs(Form("../Plots/LaneStatusFlag_%s.pdf", filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
		if(iLayer==NLayer-1)ctrend3[iLayer]->SaveAs(Form("../Plots/LaneStatusFlag_%s.pdf]", filepath.substr(filepath.find("from"), filepath.find(".root")-filepath.find("from")).c_str()));
	}//iLayer
